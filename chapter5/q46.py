        ...
        for chunk in sentence:
            ...
            for src_idx in chunk.srcs:
                ...
                if particle:
                    ...
                    particle_arg_pairs.append((particle, argument_text))

            if len(particle_arg_pairs) > 0:
                particle_arg_pairs.sort(key=lambda x: x[0])
                formatted_particles = [p[0] for p in particle_arg_pairs]
                formatted_args = [p[1] for p in particle_arg_pairs]
                f.write(f"{predicate}\t{' '.join(formatted_particles)}\t{' '.join(formatted_args)}\n")